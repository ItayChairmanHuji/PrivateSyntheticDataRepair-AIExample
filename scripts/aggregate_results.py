        ...
        for file in files:
            if file.endswith(".json") and file.startswith("result_"):
                ...
                try:
                    ...
                    # Metadata extraction (flattened)
                    metadata = data.get("metadata", {})
                    flattened_meta = flatten_dict(metadata, parent_key="meta")
                    row.update(flattened_meta)
                    
                    # Derived columns for easier filtering
                    row["repairer"] = metadata.get("repairer")
                    exp_name = data.get("experiment_name")
                    if exp_name == "weighted_fix_test":
                        row["repairer"] = f"{row['repairer']}_fixed"
                    elif exp_name == "weighted_fix_v2":
                        row["repairer"] = f"{row['repairer']}_v2"
