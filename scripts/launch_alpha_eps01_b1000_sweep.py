...
def generate_alpha_eps01_b1000_sweep():
    ...
    for ds in datasets:
        for model in models:
            for alpha in alphas:
                ...
                exp_name = f"alpha_eps01_b1000_{ds}_{model}_a{alpha}"
                override = (
                    f"loading={ds} "
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Launch alpha sweep with eps=0.1 and budget=1000.")
    parser.add_argument(
    ...
    overrides = generate_alpha_eps01_b1000_sweep()
    ...
    if args.canary:
        ...
        canary_exp = overrides[0]
        canary_exp = canary_exp.replace(
            "experiment_name=", "experiment_name=canary_alpha_eps01_b1000_"
        )
