    ...
    for dataset in datasets:
        for engine in engines:
            for eps in epsilons:
                ...
                exp_name = (
                    f"weighted_updated_eps_v4_{dataset}_{engine}_eps{eps}_a{alpha}"
                )
...
def generate_alpha_eps01_sweep():
    datasets = ["adult", "census", "compas", "tax"]
    models = ["aim", "mst"]
    alphas = [round(i * 0.05, 2) for i in range(21)]
    eps = 0.1
    ...
    for ds in datasets:
        for model in models:
            for alpha in alphas:
                ...
                exp_name = f"weighted_alpha_eps01_v4_{ds}_{model}_a{alpha}"
                override = (
                    f"loading={ds} "
                    f"synthesizing=model_loader "
                    f"synthesizing.model_path={model_path} "
...
def main():
    ...
    parser.add_argument(
        "--canary", action="store_true", help="Run a single canary experiment on remote"
    )
    ...
    parser.add_argument("--alpha-eps01", action="store_true", help="Launch alpha sweep with eps=0.1")

    args = parser.parse_args()

    if not args.alpha and not args.epsilon and not args.alpha_eps01:
        print("Please specify --alpha, --epsilon, --alpha-eps01 or both.")
        return
    ...
    if args.alpha_eps01:
        overrides.extend(generate_alpha_eps01_sweep())
    ...
    if args.canary:
        ...
        canary_exp = canary_exp.replace(
            "experiment_name=", "experiment_name=canary_weighted_v4_"
        )
    ...
    group_name = "weighted_updated_may15"
