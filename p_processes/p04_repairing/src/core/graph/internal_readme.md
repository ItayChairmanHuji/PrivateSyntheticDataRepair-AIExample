...
- `initializer.py`: Calculates initial vertex degrees and builds the "Phonebook" (Cluster -> Blocks).
- `graph.py`: The runtime engine. Manages vertex degrees and the active/deleted masks.
- `cluster_map.py`: Manages the mapping between rows and clusters.
- `vertex_seq.py`: Provides compatibility for algorithms expecting vertex-level selectors.
...
3.  **Phonebook**: Each cluster stores a list of `BlockMembership` objects pointing to the blocks it participates in and its role (`side`).

### B. Efficient Vertex Removal
When `vertex_i` (in cluster $C$) is removed:
1.  **Direct Lookup**: Jump to the specific blocks connected to cluster $C$ via the Phonebook.
