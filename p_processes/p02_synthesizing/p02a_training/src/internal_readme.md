...
- **Responsibility**: Atomic training core.
...
### 3. The Orchestrator (`src/orchestration/training_worker.py`)
...
## Data Flow
1. `main.py` instantiates the `TrainingWorker`.
2. Orchestrator calls `Engine.load_dataset()`.
3. Orchestrator calls `Worker.train()`.
4. Orchestrator calls `Engine.get_model_path()` to find the destination.
5. Orchestrator calls `Engine.save_model()`.
