...
- **Testing**: Each implementation should be verified with a corresponding notebook in the `/notebooks` directory. **A notebook MUST be created or updated for every new feature to ensure no regressions.**
...
- **Orchestration Pattern**: Try to use an orchastration pattern when possible- classes that have logic and classes that combines these classes together. Try to avoid combining the classes. 
