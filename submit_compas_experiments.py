...
param_file = Path("compas_experiment_params.txt")
...
Path("run_compas_exp.sh").write_text(wrapper_script)

print("Generated run_compas_exp.sh and compas_experiment_params.txt.")
if len(jobs) > 1000:
    print("Large job detected. Submit in parts:")
    for offset in range(0, len(jobs), 1000):
        end = min(offset + 999, len(jobs) - 1)
        count = end - offset + 1
        print(f"sbatch --export=OFFSET={offset} --array=0-{count-1} run_compas_exp.sh")
else:
    print(f"To submit: sbatch --array=0-{len(jobs)-1} run_compas_exp.sh")
