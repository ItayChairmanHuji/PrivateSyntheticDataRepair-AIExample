...
class DCsEncoder:
    ...
    def _encode_dc(self, dc: DenialConstraint, mappings: dict[str, LabelEncoder]) -> DenialConstraint:
        encoded_predicates = [self._encode_predicate(p, mappings) for p in dc.predicates]
    ...
    def _encode_predicate(self, p: Predicate, mappings: dict[str, LabelEncoder]) -> Predicate:
        new_left = p.left # Side itself doesn't change if it's an attribute
        new_right = p.right
        
        new_left, new_right = self._handle_value_encoding(p, new_left, new_right, mappings)
